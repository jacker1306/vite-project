<template>
    <h3 class="text-3xl mt-16 mb-2">PRODUCT LISTING</h3>
    ...
    <div class="w-4/5 flex flex-wrap mx-auto justify-between items-stretch mt-8">
        <a  href="#" v-for="fashion in fashions.slice(0,4)" :key="fashion.id" class="w-1/5 mx-1 my-3 flex flex-col justify-between">
            <div class="fashion-img flex items-center">
                <img :src="fashion.img" alt="h-full">
            </div>
            <div>
                <p class="mt-4">{{fashion.title}}</p>
                <p class="text-red-600">{{fashion.price}}</p>
                <p>{{fashion.variants}}</p>
            </div>
        </a>
    </div>
